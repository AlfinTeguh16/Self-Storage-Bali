...
@section('title', 'Create Booking (Customer)')
@section('content')

<h1 class="text-3xl font-bold mb-6 text-gray-800">Create Booking</h1>

<form action="{{ route('data-booking.store') }}" method="POST" class="space-y-6">
    @csrf
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <x-form type="select" name="storage_id" label="Storage Unit" required="true">
                ...
                @foreach($storageUnits as $unit)
                    <option value="{{ $unit->id }}" {{ old('storage_id') == $unit->id ? 'selected' : '' }}>
                        {{ $unit->unit_code }} — Size: {{ $unit->size }}
                    </option>
                @endforeach
            </x-form>

            {{-- Booking Ref (opsional, bisa auto-generate di backend) --}}
            <x-form
                name="booking_ref"
                label="Booking Ref"
                type="text"
                placeholder="Masukkan Kode Booking / biarkan kosong untuk auto"
